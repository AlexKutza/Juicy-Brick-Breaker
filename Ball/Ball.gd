...
func _on_Ball_body_entered(body):
	...
	$Images/Highlight.modulate.a = 0.5
	tween.tween_property($Images/Highlight, "modulate:a", 0, time_highlight)
	$Images/Highlight.scale = Vector2(0.2,0.2)
	tween.tween_property($Images/Highlight, "scale", Vector2(0.2,0.2), time_highlight_size).set_trans(Tween.TRANS_BOUNCE).set_ease(Tween.EASE_IN)
